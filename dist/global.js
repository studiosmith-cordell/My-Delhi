	...
	'node_modules/gsap/dist/ScrollTrigger.js'(exports, module) {
		(function (global, factory) {
			...
			var Observer2 = (function () {
				...
				_proto.init = function init4(vars) {
					...
					var id,
						...
						passive =
							vars.passive ||
							(!preventDefault && vars.passive !== false),
						...
						_onDrag = function _onDrag2(e2) {
							...
							if (
								isDragging ||
								((dx || dy) &&
									(Math.abs(self2.startX - x) >=
										dragMinimum ||
										Math.abs(self2.startY - y) >=
											dragMinimum))
							) {
					...
					(force === true ||
						(!_refreshing &&
							...
							(!_ignoreMobileResize ||
								_baseScreenWidth !== _win$1.innerWidth ||
								Math.abs(
									_win$1.innerHeight - _baseScreenHeight
								) >
						...
						if (invalidateOnRefresh && animation) {
							...
							animation.getChildren &&
								animation
									...
									.forEach(function (t2) {
										return (
											t2.vars.immediateRender &&
											t2.render(0, true, true)
										);
									});
						}
			...
			ScrollTrigger$1.sort = function (func) {
				...
				ScrollTrigger$1.getAll().forEach(function (t2) {
					return (t2._sortY = t2.trigger
						? scroll + t2.trigger.getBoundingClientRect().top
						: t2.start + _win$1.innerHeight);
				});
	...
	'node_modules/gsap/dist/SplitText.js'(exports, module) {
		(function (global, factory) {
			...
			let gsap2,
				...
				_coreInitted2,
				_initIfNecessary = () =>
					_coreInitted2 || SplitText2.register(window.gsap),
				_charSegmenter =
					typeof Intl !== 'undefined' ? new Intl.Segmenter() : 0,
				_toArray = (r) =>
					typeof r === 'string'
						...
						: [r],
				_elements = (targets) =>
					_toArray(targets).filter((e2) => e2 instanceof HTMLElement),
				_emptyArray2 = [],
				_context2 = function () {},
				_spacesRegEx = /\s+/g,
				...
				_stretchToFitSpecialChars = (collection, specialCharsRegEx) => {
					if (specialCharsRegEx) {
						let charsFound = new Set(
								collection.join('').match(specialCharsRegEx) ||
									_emptyArray2
							),
							i = collection.length,
							slots,
				...
				_getWrapper = (type, config3, collection) => {
					let className = config3[type + 'sClass'] || '',
						{
							tag = 'div',
							aria = 'auto',
							propIndex = false,
				...
				_getLineWrapper = (element, nodes, config3, collection) => {
					let lineWrapper = _getWrapper('line', config3, collection),
						textAlign =
				...
				_splitWordsAndCharsRecursively = (
					...
					if (typeof wordDelimiter === 'object') {
						wordDelimSplitter =
							wordDelimiter.delimiter || wordDelimiter;
						...
						if (curNode.nodeType === 3) {
							...
							words =
								wordDelimString || wordDelimSplitter
									? curTextContent.split(
											wordDelimSplitter || wordDelimString
									  )
							...
							for (j = 1; j <= words.length; j++) {
								...
								if (
									...
									(_a = curNode.previousSibling) == null
										? void 0
										: _a.remove();
									_insertNodeBefore(
								...
								if (!reduceWhiteSpace && wordText === '') {
									...
									if (
										...
										wordsCollection.indexOf(
											ignoredPreviousSibling.parentNode
										) > -1
									) {
										curWordEl =
											wordsCollection[
												wordsCollection.length - 1
											];
										curWordEl.appendChild(
											document.createTextNode(
												charWrapper ? '' : wordText
											)
										);
									} else {
										curWordEl = wordWrapper(
											charWrapper ? '' : wordText
										);
										_insertNodeBefore(
											curWordEl,
									...
									if (deepSlice && isNested) {
										curTextContent = curNode.textContent =
											curTextContent.substring(
												wordText.length + 1,
												curTextContent.length
											);
										bounds =
											curWordEl.getBoundingClientRect();
										if (
											...
											while (
												curSubNode &&
												curSubNode !== curWordEl
											) {
												tempSubNode = curSubNode;
												curSubNode =
													curSubNode.nextSibling;
				...
				split(config3) {
					...
					let {
						...
						finalCharSplitRegEx = specialCharsRegEx
							? new RegExp(
									specialCharsRegEx.source +
										'|' +
										_emojiSafeRegEx.source,
									'gu'
							  )
							: _emojiSafeRegEx,
						ignore = !!config3.ignore && _elements(config3.ignore),
						...
						this.elements.forEach((element, index) => {
							...
							let chars = [],
								...
								charWrapper = splitCharacters
									? _getWrapper('char', config3, chars)
									: null,
								wordWrapper = _getWrapper(
									'word',
									config3,
									words
								),
							...
							_splitWordsAndCharsRecursively(
								...
								onlySplitCharacters,
								deepSlice &&
									(splitLines || onlySplitCharacters),
								ignore,
								finalCharSplitRegEx,
								specialCharsRegEx,
								false
								...
								for (i = 0; i < words.length; i++) {
									...
									if (
										...
										if (smartWrap && !splitLines) {
											...
											smartWrapSpan.style.whiteSpace =
												'nowrap';
											while (curWord.firstChild) {
												smartWrapSpan.appendChild(
													curWord.firstChild
												);
											}
...
var import_SplitText = __toESM(require_SplitText());
...
var split = import_SplitText.SplitText.create('[text-split]', {
	type: 'words, chars',
	wordsClass: 'word',
	charsClass: 'char',
	mask: 'words',
